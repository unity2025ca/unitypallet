...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/admin/settings/:category", requireAdmin, async (req: Request, res: Response) => {
    try {
  ...
  app.get("/api/admin/site-config", requireAdmin, async (req: Request, res: Response) => {
    try {
      const settings = await storage.getAllSettings();
      const config = {
        general: settings.filter(s => s.category === 'general'),
        contact: settings.filter(s => s.category === 'contact'),
        appearance: settings.filter(s => s.category === 'appearance'),
        content: settings.filter(s => s.category === 'content'),
        business: settings.filter(s => s.category === 'business'),
  ...
  app.get("/api/admin/system-info", requireAdmin, async (req: Request, res: Response) => {
    try {
      const products = await storage.getAllProducts();
